...
function NonAdminProfile() {
  ...
  const { firstname, lastname, phone, email, addressline, street, city, state, postcode, country , companyname} = value
  ...
  const fetchData = () => {
    try {
      ...
      fetch(`https://solar365.co.in/get_none_admin_profile/${location?.state?.ele?.admin?.user?.id}/`, requestOptions)
  ...
  const updateProfile = (e) => {
    ...
    try {
      ...
      formdata.append("company_name", companyname !== "" ? companyname : profile?.company_name)
      ...
      fetch(`https://solar365.co.in/register/${location?.state?.ele?.admin?.user?.id}/`, requestOptions)
        ...
        .then(result => {
          if (result?.message === "success") {
            ...
            return navigate('/register-non-admin')
          }
        })
        .catch(error => console.log('error', error));
    } catch (error) {
